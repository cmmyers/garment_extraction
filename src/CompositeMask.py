...
class CompositeMask():
    def __init__(self, filename, bg_threshold=0.4, face_threshold=0.1,
        ...
        bg_pal = self.get_background_one_image(num_bg_colors)
        # change these colors to decimal representation
        self.bg_color_list = [
            [c[0] / 255, c[1] / 255, c[2] / 255] for c in bg_pal]
    ...
    def get_background_one_image(self, num_points):
        background_colors = []
        h, w, colors = self.image.shape

        for i in range(int(num_points / 2)):
            row = np.random.choice(h)
            col = np.random.choice(int(w / 8))
            color_block = self.image[int(
                row - 4):int(row + 4), int(col - 4):int(col + 4)]
            ave_color = [self.avg_each_color(color_block, color) for color in [
                'red', 'green', 'blue']]
    ...
    def make_mask_bg(self, three_colors):
        lower = np.array([(c - self.bg_threshold * c)
                          * 255 for c in three_colors])
        upper = np.array([(c + self.bg_threshold * c)
                          * 255 for c in three_colors])
    ...
    def find_ratio_face_to_all_non_bg(self):
        ...
        body_pixels = self.make_boolean_mask_from_segs(
            ['accessories', 'skin', 'garment'])
    ...
    def remove_bg_colors_from_face_pal(self):
        ...
        for i, c1 in enumerate(self.face_color_list1):
            for j, c2 in enumerate(self.bg_color_list):
                e = euclidean_distances(np.array(c1).reshape(
                    1, -1), np.array(c2).reshape(1, -1))
                if e < 0.2:
                    bg_colors.append(c1)
                break
    ...
    def get_contours(self):
        ...
        self.contoured_img, self.contours, self.hierarchy =    \
            cv2.findContours(shapeMask.copy(), cv2.RETR_EXTERNAL,
    ...
    def make_boolean_mask_from_contours(self):
        ...
        self.contour_mask = cv2.inRange(
            self.im_largest_contoured, low_bg, high_bg)
